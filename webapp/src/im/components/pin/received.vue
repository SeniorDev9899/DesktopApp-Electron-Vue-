    ...
    <div class="rong-component">
        ...
        <div v-else-if="!showEmptyPage" class="rong-pin-list">
            <ul class="rong-main-content clearfix" @scroll="scroll()" v-rong-scroll-bar-y>
                <li v-for="(pin, index) in pinList" :key="index" class="rong-pin-item" @click.prevent="showDetail(pin);"
                @contextmenu.prevent="showContextmenu($event, {pin: pin});isClicking=true;" >
                    <div class="rong-pin-item-inner">
                        ...
                        <div class="rong-pin-item-hd">
                            ...
                            <a @click.prevent="userProfile(pin.creator_uid)" :title="getUsername(pin.user)" href="">{{ getUsername(pin.user) }}</a></div>
                            <div>
                                <span class="rong-pin-item-datetime">{{dateFormat(pin.send_dt)}}</span>
                                <i v-if="pin.attachment_count" class="rong-pin-attachment">
        ...
        <transition name="rong-setting-slide" v-on:after-enter="afterEnter"
            v-on:enter-cancelled="enterCancelled">
            <pinDetail v-if="selectedPin" v-on:hidepanel="hidepanel"
                v-on:receiveComment="selectedPin.un_read_comment_count=0"
                v-on:confirmPin="selectedPin.confirmed=true" :isReply="selectedPin.isReply"
                :isSender=false :pinDetail="selectedPin">
            </pinDetail>
        </transition>
