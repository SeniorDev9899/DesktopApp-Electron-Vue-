/**
 * RongRTC.js v3.2.9
 * CodeVersion: 9505baabedb88511487cee0556374df833a14276
 * Release Date: Thu Apr 22 2021 09:56:10 GMT+0800 (China Standard Time)
...
(function (global, factory) {
  ...
  */
  ...
  var getMCUConfigUrl = function getMCUConfigUrl(configUrl) {
    return configUrl.replace(/^(https?:\/\/)?/, 'https://');
  };
  ...
  */
  ...
  var PeerConnection = function (_EventEmitter) {
    ...
    createClass(PeerConnection, [{
      ...
      value: function setBitrate(answer) {
        ...
        var start = bitrate.start || Math.floor(maxValue * .7);
  ...
  var getIMEventsByV2 = function getIMEventsByV2(option) {
    ...
    return {
      ...
      setInnerUserData: function setInnerUserData(id, key, value, isInner, message) {
        return utils.deferred(function (resolve, reject) {
          instance.setRTCUserData(id, key, value, isInner, {
            onSuccess: resolve,
            onError: reject
  ...
  var getIMEventsByV3 = function getIMEventsByV3(option) {
    ...
    return {
      ...
      setInnerUserData: function setInnerUserData(id, key, value, isInner, message) {
        var rtc = new im.RTC({ id: id });
        // __setUserData 为 imlib 3.0 中的定义，因 setUserData 已被占用；setUserData 为 imlib 4.0 方法
        // eslint-disable-next-line no-underscore-dangle
        return rtc.__setUserData ? rtc.__setUserData(key, value, isInner, message) : rtc.setUserData(key, value, isInner, message);
      },
      getRTCRoomData: function getRTCRoomData(id, keys, isInner) {
        var rtc = new im.RTC({ id: id });
  ...
  var IM = function (_EventEmitter) {
    ...
    createClass(IM, [{
      ...
      key: 'setInnerUserData',
      value: function setInnerUserData(key, value, isInner, message) {
        var id = this.room.id,
            im = this.im;

        return im.setInnerUserData(id, key, value, isInner, message);
      }
    }, {
      key: 'getRoomData',
  ...
  function StorageHandler(im) {
    var getType = function getType(type) {
      if (utils.isEqual(type, StorageType.ROOM)) {
        return 'Room';
      } else if (utils.isEqual(type, StorageType.USER)) {
        return 'User';
      } else {
        return 'InnerUser';
      }
    ...
    var set$$1 = function set$$1(type, key, value, message, isInner) {
      var name = getName('set', type);
      return im[name](key, value, isInner, message);
    };
    var get$$1 = function get$$1(type, key, isInner) {
      var name = getName('get', type);
      return im[name](key, isInner);
    };
    var remove = function remove(type, key, message, isInner) {
  ...
  */
  ...
  var Storage$2 = function () {
    ...
    createClass(Storage, [{
      key: '__innerSet',
      value: function __innerSet(key, value, message) {
        return this.__set(key, value, message, true);
      }
    }, {
      key: 'set',
      value: function set$$1(key, value, message) {
        return this.__set(key, value, message, false);
      }
    }, {
      key: '__set',
      value: function __set(key, value, message, isInner) {
        ...
        return client.exec({
          ...
          type: 'storage',
          args: [type, key, value, message, isInner]
        });
      }
    }, {
      key: '__innerGet',
      value: function __innerGet(key) {
        return this.__get(key, true);
      }
    }, {
      key: 'get',
      ...
      key: '__get',
      value: function __get(key, isInner) {
        ...
        return client.exec({
          ...
          type: 'storage',
          args: [type, key, isInner]
        });
      }
    }, {
      key: '__innerRemove',
      value: function __innerRemove(key, message) {
        return this.__remove(key, message, true);
      }
    }, {
      key: 'remove',
      value: function remove(key, message) {
        return this.__remove(key, message, false);
      }
    }, {
      key: '__remove',
      value: function __remove(key, message, isInner) {
